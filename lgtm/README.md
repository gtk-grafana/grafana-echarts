...
metric that the LGTM **relations** dashboards use, so the graph, sankey, and chord
panels have live data locally instead of needing a cluster. The reader accepts
`client` and `server` as an endpoint pair directly, so these dashboards do not rename
it. `dashboards/lgtm-stack.json` uses the plain aggregation with an empty Transform tab.
...
Six dashboards showing how each datasource's **real** response reaches the
...
| `prometheus-adhoc-filters.json`       | `traces_service_graph_request_total`             | **One panel per ad-hoc-filter behaviour**, because the footer's buttons only mean anything against a real datasource. The service graph is labelled `client` / `server`, a pair the reader recognises, so five of the six panels configure nothing: the gate, the direction a node may assert (`user` is a pure origin, `elasticsearch` a pure destination), per-edge key recovery across two levels, and the deduped node between them. Panel 5 is the `prometheus-derived-nodes.json` caveat again, from the filter side: a node override has no field to bind to here, which is exactly why a node falls back to the edges' opt-in. Sibling of `echarts-relations-adhoc-filters` on a dev Grafana that does have the API.                                                                                                                                                                                                                                                                                                                                                  |
...
| `prometheus-timeline-thresholds.json` | `traces_service_graph_request_total`             | **The only dashboard here where both kinds of mark move.** Two _ranged_ queries — the service graph's `client`/`server` rates for the edges, the same rate `sum by (server)` for the nodes — so the time slider re-grades nodes and edges together on every stop. The nodes query needs one filtered `labelsToFields` (Value field name: `server`) because a node id **is** a field name and every raw Prometheus value field is called `Value`; `legendFormat` + a join cannot stand in for it, since real Prometheus sets `displayNameFromDS` and leaves `frame.name` unset, which is the only thing `joinDataFrames` renames from. Absolute and percentage threshold panels side by side, the second with **Field min/max** on — without it a percentage step is a percentage of the _response's_ range and one spiky edge flattens every other mark to green. A third panel answers "is a node value ever more than the sum of its edges": nodes in `bytes` (resident memory), edges in `reqps`, one threshold set each, because a node is a stock and an edge is a flow. |
