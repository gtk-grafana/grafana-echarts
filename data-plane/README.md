...
docs describe the *current* behavior of the converters in
...
(fields + metadata). The data plane adds a *type* (`frame.meta.type`) declaring
...
| [heatmap-matrix.md](./heatmap-matrix.md) | category x category heatmap (native series)     | Numeric (wide / pivot) — *not* the Heatmap kind |
