...
the frame's kind (time series, numeric, heatmap, ...). This plugin reads data
types for the heatmap family and for relations compatibility roles.
...
The graph-wide documents separate the portable proposal from this plugin implementation:

- [graph-wide-proposed.md](./graph-wide-proposed.md): The proposed data plane type. One
  mark is one field.
- [graph-wide.md](./graph-wide.md): How the ECharts relations family reads the proposed
  shape and compatibility input.
...
The [shipped graph-wide contract](./graph-wide.md) describes how the Relations panel reads these formats.

## Models

| Doc                                                | Consumer or ECharts charts                                | Grafana kind consumed                                            |
| -------------------------------------------------- | --------------------------------------------------------- | ---------------------------------------------------------------- |
...
| [graph-wide-proposed.md](./graph-wide-proposed.md) | Proposed specification for each graph consumer            | `graph-wide` with a `nodes` or `edges` role                      |
| [graph-wide.md](./graph-wide.md)                   | ECharts graph, sankey, and chord implementation           | `graph-nodes-wide` / `graph-edges-wide` (one mark = one field)   |
