 ...
 * nothing.
 *
 * **A last resort, and a narrow one.** Three routes reach the right key before this does,
 * and between them they cover every query that has not thrown the key away:
 *
 ...
 *   edge** (`aliasEndpointKeys`). That is the only route a multi-level flow has, since its
 *   levels relabel from different originals and no single setting can hold two answers;
 * - a **node** takes its keys from the edges touching it (`nodeFilterLabels`), so nothing
 *   has to be configured per node either.
 *
 * These two are for the query that cannot be rewritten, where the key really is gone.
 *
