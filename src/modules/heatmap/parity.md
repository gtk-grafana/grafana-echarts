...
| Legend (`legend.show`)                                                   | Grafana legend via `addLegendOptions` (governs the cartesian **overlay** series, not the cells); interactive show/hide + color persist as `byName` overrides | Partial / different scope |
