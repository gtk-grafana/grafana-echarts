...
| Legend: slice show/hide + color (interactive) | Per-slice toggle; converter reads `byName` overrides by category | Supported     |
...
| `legend`                                                                                                 | Supported       | Grafana DOM legend (`addLegendOptions`); native legend hidden. Interactive per-slice show/hide + color read from `byName` overrides by category. |
