...
| ------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| Value / calculation (data reduce)                 | `reduceOptions` (Calculate/All values + Calculation) via `addStandardDataReduceOptions`                                                | Supported       | [canvas: Calculate sum — one slice per numeric field][canvas], [canvas: Calculate mean — same fields, different slice sizes][canvas], [unit: resolvePieSlices][pie-conv]                                | [pie-parity.json][db-parity] · [#2 live][live-parity-2]              |
| Fields to include, limit                          | `reduceOptions.fields` / `reduceOptions.limit`                                                                                         | Supported       | [canvas: All values with limit — one slice per row, capped][canvas], [unit: caps the slices at the configured limit][pie-conv]                                                                          | —                                                                    |
...
| Arc start / end angle (ECharts-only)              | `startAngle` / `endAngle` number inputs in the "Pie" category (Advanced); half-pie / semicircle donut via `getPieAngles`               | Advanced        | [canvas: half-pie (start 180 / end 360)][canvas], [canvas: semicircle donut (donut + start 180 / end 360)][canvas], [unit: getPieAngles][pie-opts]                                                      | [pie-angles.json][db-angles] · [#2 live][live-angles-2]              |
...
| -------------------------------------- | -------------------------------------------------------------------------------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| Rose type (Nightingale)                | `roseType` (Pie category, Radius / Area); rendered by `getPieRoseType`                             | Advanced | [canvas: radius (value as slice radius)][canvas], [canvas: area (value as slice area)][canvas], [unit: getPieRoseType][pie-opts]                                                                        | [pie-rose-type.json][db-rose] · [#2 live][live-rose-2]              |
...
| Label position                         | `labelPosition` → `label.position` (Outside / Inside / Center); threaded into `getPieContentLabel` | Advanced | [canvas: inside (labels on the slices)][canvas], [canvas: center on a donut without a reducer (empty until hover)][canvas], [unit: getPieContentLabel — threads the inside / center position][pie-opts] | [pie-label-position.json][db-labelpos] · [#2 live][live-labelpos-2] |
| Center value (donut readout)           | `centerValueReducer` (shown only with center labels); drives the donut-center `title`              | Advanced | [canvas: center on a donut without a reducer (empty until hover)][canvas], [unit: getPieCenterTitle, getPieCenterEmphasisLabel][pie-opts]                                                               | [pie-label-position.json][db-labelpos] · [#3 live][live-labelpos-3] |
...
| -------------- | -------------------------------------------- | ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
| Color scheme   | Kept (bySeries, gradient, seeded fixedColor) | Kept (PaletteClassic, byValue + bySeries) | [canvas: byName fixed-color override][canvas], [canvas: by-value color scheme (All values)][canvas], [unit: byName parity][pie-conv] | [legend-visibility-color.json][db-legvis] · [#2 live][live-legvis-2]                                           |
