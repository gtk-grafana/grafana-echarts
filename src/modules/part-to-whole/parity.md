...
- Editor options are tiered via the shared `editorMode` option (Default =
  parity-only, Advanced = ECharts extras, API = JSON-only). Core-parity pie
  options are Default; the first ECharts-only extra, **Label position**
  (`labelPosition` → `label.position`: Outside / Inside / Center), is gated to
  Advanced via `showIf: isAdvancedEditorMode`. See
...
| -------------------------------------------------------------------------------------------------------- | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `series` (pie)                                                                                           | Partial         | `seriesType: pie`; slice labels (Name/Value/Percent) via `label`; label placement (Outside/Inside/Center) via `label.position` (Advanced `labelPosition`); pie/donut via `radius`; sorting via the resolver; center offset not exposed. |
