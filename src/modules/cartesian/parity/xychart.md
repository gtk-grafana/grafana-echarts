...
| —                                              | Advanced: `performance.showPoints` / `performance.downsampling` (line-only), `animation.enabled` (off by default); `scatter` auto-enables `large` at 2,000+ points/series (`effectScatter` untouched) | Advanced      |
...
| `tooltip`                                                                                      | Supported       | Rendered by a React `@grafana/ui` `VizTooltip` overlay; mode maps to `trigger` (item / axis / none). Supports pin, data links, ad-hoc filters. |
