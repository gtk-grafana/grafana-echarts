...
| —                                              | Advanced: `performance.showPoints` / `performance.downsampling` (line-only), `performance.animation`; `scatter` auto-enables `large` at 2,000+ points/series (`effectScatter` untouched) | Advanced      |
