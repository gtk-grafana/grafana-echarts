...
| Safety limits           | All           | Rejects data before ECharts     | Different implementation   | [limit unit][mark-limits], [chart module unit][rel-chart]                                                                                                                                                                                                                                                                                                                                                                      | [performance-limits.json][db-performance-limits]                              |
...
Presets appear in this order: Service topology, Circular network, Weighted flow, Mutual relations, and Time network. A preset is hidden when its automatic node or edge safety budget is exceeded. Circular network permits an unknown node count or 2 through 40 nodes. Time network needs ranged time data and a known node count from 2 through 40. Presets show the legend for an unknown node count or up to 12 nodes. They hide it from 13 nodes. Time network puts the legend on the right. The other presets put it at the bottom. [The preset unit tests pin these rules][preset-tests].
...
| Max nodes + edges | All                 | Replaces the automatic node and edge limits | [limit unit][mark-limits], [editor unit][performance-editor]                                                                                                                                                                 | [performance-limits.json][db-performance-limits]         |
...
| Filterable                      | Yes           | Ad hoc filters                    | [filter button test][filters-test], [tooltip overlay test][tip-overlay], [tooltip filter unit][tip-filters]                                                                                                            | [per-mark-tooltip-links.json][db-marks]                                          |
...
| Node radius                     | Override only | Graph                             | [graph canvas][canvas-graph], [conversion unit][wide-conv]                                                                                                                                                             | [graph-wide.json][db-wide]                                                       |
...
| Source and target filter labels | Deprecated    | Keep original labels in the query | [tooltip mark unit][tip-marks], [filter button test][filters-test], [derived conversion unit][derived-conv]                                                                                                            | [lgtm-stack.json][db-lgtm-stack]                                                 |
...
[mark-limits]: ../../lib/echarts/relations/markLimits.test.ts
[performance-editor]: ../../lib/grafana/editor/relations/performance.test.ts
...
[db-lgtm-stack]: ../../../lgtm/provisioning/dashboards/lgtm-stack.json
...
[db-performance-limits]: ../../../provisioning/dashboards/relations/performance-limits.json
