...
> What remains is phase 6 (docs and provisioning) plus the genuinely open items below,
> chiefly a node _derived_ from an edge's endpoints, which has no field for any of this to
> hang on. The **[wide: …]** markers are the target;
...
| Unit           | **Yes**          | Per mark, not per frame: a hovered node or edge formats with its own field's unit, in the tooltip **and** in the "Show node values" label. Two nodes of one graph can differ, which the row form cannot express.                                                                                                                                                | [unit: per-mark formatting][tip-marks], [e2e: emitted model][tip-emit]                                                            | [per-mark-tooltip-links.json][db-marks] |
...
| Data links     | **Yes**          | The pinned footer resolves the hovered mark's **own** field, so a `byName` `links` override puts a link on exactly one node or one edge and leaves the others with no footer at all. A node _derived_ from an edge's endpoints has no field, so it still shows none.                                                                                            | [e2e: node, edge and the marks without a link][dl-test]                                                                           | [per-mark-tooltip-links.json][db-marks] |
...
| **Data links** — "nodes _derived_ from the edges frame carry no row, so no footer"       | **Shipped.** Per mark via `config.links`, so a link lands on one node or one edge. Derived nodes stay **partially open** — no field either                                                     |
