...
- **Regression test** — the automated test that pins the behaviour, cited by its **exact
  test name**, with the link naming the file it lives in. The names are checked against
  the source on every run (`src/test/parityCitations.test.ts`), so a renamed or deleted test
  fails this table rather than quietly outliving it.
  - `canvas:` — a case in one of the four `relations-*.canvas.test.tsx` suites, whose
    assertion _is_ a recorded set of draw calls reviewed as an image. The parenthetical
    in the name is what that picture shows.
  - `integration:` — a case in one of the four `relations-*.integration.test.tsx`
    suites: a claim about the relation between two renders, or about which labels
    survived, which commits no baseline because a stored picture states it worse.
  - `unit:` — an option-mapping or converter test.
  - "needs e2e" marks an option neither a picture nor a draw-call comparison can prove
    — the Grafana DOM legend, tooltip content, pan/zoom and drag gestures — which needs
    a [`@grafana/plugin-e2e`](../../../tests/panel.spec.ts) test instead; none of those
    are written yet. `—` means no coverage of any kind.
...
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Layout algorithm (Layered/Force/Grid)         | "Layout" (Force / Circular / Fixed) — `series.graph.layout`                                                                    | Partial / different set     | [canvas: fixed coordinates from the data (nodes at the server's x and y, not on a ring)][canvas-graph] (Fixed), [integration: every node is drawn even when the data pins nothing][int-layout], [unit: getGraphLayout][graph-layout]; `force` is deliberately not snapshotted                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | [node-graph-testdata.json][db-testdata] · [#6 live][live-testdata-6] (Circular) · [#10 opts][live-opt-10]                                                                            |
| Zoom mode (Cooperative/Greedy)                | "Zoom" — panel buttons + the `graphRoam` / `sankeyRoam` action (Default, off)                                                  | Partial                     | [integration: the roam action scales the view while scroll-to-zoom stays off][int-interaction], [unit: getZoomAction][rel-chart], [unit: resolveRelationsZoom][graph-view]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | [readability.json][db-read] · [#8 live][live-read-8] · [#15 opts][live-opt-15]                                                                                                       |
| —                                             | "Pan" — `series.*.roam: 'move'` (Default, off)                                                                                 | ECharts-only                | [unit: resolveRelationsRoam][graph-view], [unit: emits move roam when panning is on][graph-opts], [unit: honors the interaction switches][sankey-opts]; the drag itself needs e2e                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | [readability.json][db-read] · [#8 live][live-read-8] · [#32 opts][live-opt-32]                                                                                                       |
| —                                             | "Remember view" — persists the roamed view into the panel options                                                              | ECharts-only                | unit-covered via `getRelationsViewState`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | —                                                                                                                                                                                    |
...
| —                                             | "Show node labels" — `series.graph.label.show`                                                                                 | ECharts-only                | [canvas: node labels off (symbols and links, no text)][canvas-graph], [canvas: node labels off (bars and ribbons, no text)][canvas-sankey], [canvas: node labels off (arcs and chords, no text)][canvas-chord], [unit: getGraphLabel][graph-labels], [unit: getSankeyLabel][sankey-opts], [unit: getChordLabel][chord-opts]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | [all-options.json][db-opts] · [#4 opts][live-opt-4]                                                                                                                                  |
| —                                             | "Show node values" — appends `calcs[0]` to the node label                                                                      | ECharts-only                | unit-covered via `getRelationsNodeLabelFormatter`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | [all-options.json][db-opts] · [#5 opts][live-opt-5]                                                                                                                                  |
| —                                             | "Node size" — `series.graph.symbolSize`                                                                                        | ECharts-only                | [unit: getGraphSeries — relationsNodeSize][graph-opts]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | [all-options.json][db-opts] · [#11 opts][live-opt-11]                                                                                                                                |
| —                                             | "Draggable nodes" — `series.graph.draggable` (Advanced)                                                                        | ECharts-only                | [unit: keeps roam and draggable off by default][graph-opts], [unit: pins draggable and roam off][sankey-opts]; the drag itself needs e2e                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | [all-options.json][db-opts] · [#33 opts][live-opt-33]                                                                                                                                |
| —                                             | Repulsion / Edge length / Gravity — `series.graph.force.*` (Advanced)                                                          | ECharts-only                | [unit: getGraphForce][graph-layout]; the layout itself is only asserted to be _reproducible_ ([integration: two renders of the same frames draw identical calls][int-layout])                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | [readability.json][db-read] · [#10 live][live-read-10] · [#12 opts][live-opt-12] · [#13 opts][live-opt-13] · [#14 opts][live-opt-14]                                                 |
| —                                             | "Animate layout" — `series.graph.force.layoutAnimation` (Advanced)                                                             | ECharts-only                | [unit: getGraphForce][graph-layout]; the settling is a timed animation and needs e2e                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | [readability.json][db-read] · [#10 live][live-read-10]                                                                                                                               |
| —                                             | "Edge arrows" — `series.graph.edgeSymbol` (Default, **on**)                                                                    | ECharts-only                | [canvas: arrows off (plain lines, no heads)][canvas-graph], [unit: getGraphEdgeSymbol][graph-opts]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | [readability.json][db-read] · [#2 live][live-read-2] · [#17 opts][live-opt-17]                                                                                                       |
| —                                             | "Show edge values" — `series.*.edgeLabel` (Default, off; filed under Labels)                                                   | ECharts-only                | [canvas: edge values on (a weight drawn at each link's midpoint)][canvas-graph], [canvas: edge values on (a weight drawn on each ribbon)][canvas-sankey], [unit: getRelationsEdgeLabel][graph-labels]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | [readability.json][db-read] · [#8 live][live-read-8] · [#9 opts][live-opt-9]                                                                                                         |
| —                                             | "Link curveness" — `lineStyle.curveness` (Advanced)                                                                            | ECharts-only                | [canvas: curveness 0.3 (links bowed away from the straight line)][canvas-graph], [unit: getGraphLinkStyle][graph-opts]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | [all-options.json][db-opts] · [#18 opts][live-opt-18]                                                                                                                                |
...
| —                                             | "Hide overlapping labels" — `series.labelLayout.hideOverlap` (Default, **on**)                                                 | ECharts-only                | [integration: a node label that would collide with one already drawn is dropped][int-labels], [integration: a chord ring of collapsed arcs drops the labels that stack up][int-labels], [unit: getRelationsLabelLayout][graph-labels]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | [readability.json][db-read] · [#2 live][live-read-2] vs [#3 live][live-read-3] · [#6 opts][live-opt-6]                                                                               |
| —                                             | "Label overflow" (Default) / "Label width" (Advanced) — `label.overflow` / `label.width`                                       | ECharts-only                | [integration: a long name is cut at the label width and ends in an ellipsis][int-labels], [integration: break mode wraps a long name over several lines instead of cutting it][int-labels], [unit: getRelationsLabelStyle][graph-labels]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | [readability.json][db-read] · [#2 live][live-read-2] vs [#3 live][live-read-3] · [#7 opts][live-opt-7] · [#8 opts][live-opt-8]                                                       |
| —                                             | "Link color" (Source/Target/Gradient) — resolved per edge (Default)                                                            | ECharts-only                | [canvas: link color by endpoint (each line takes one end's colour)][canvas-graph], [canvas: gradient link color on a fixed layout (each line blends its source colour into its target)][canvas-graph], [integration: a gradient link colour is emitted only where the layout knows the positions][int-layout], [unit: resolves the source and target modes to the endpoint colours][graph-opts]. **Gradient needs the Fixed layout** — see below                                                                                                                                                                                                                                                                                                                                                                                             | [readability.json][db-read] · [#5 live][live-read-5] vs [#6 live][live-read-6] · [#16 opts][live-opt-16]                                                                             |
| —                                             | Grafana legend (`addLegendOptions`)                                                                                            | ECharts-only                | [unit: buildLegendItems — one entry per node, stable keys, swatch color][rel-chart], [unit: useLegend][use-legend]; the rendered DOM legend needs e2e                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | [node-graph-testdata.json][db-testdata] · [#6 live][live-testdata-6] · [#31 opts][live-opt-31]                                                                                       |
...
| —                                             | Animation — `animation.enabled` (Advanced, off; **sankey/chord only**)                                                         | ECharts-only                | [unit: hidden on graph, offered on sankey/chord][rel-surface]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | [readability.json][db-read] · [#15 live][live-read-15]                                                                                                                               |
| —                                             | "Time slider" — read every mark at one timestamp, stepped by hand (Default tier)                                               | ECharts-only                | [canvas: the graph at the earliest timestamp, under the slider strip (edge weights 1 and 10)][canvas-timeline], [integration: builds a different graph at an earlier timestamp than the reducer draws][int-timeline], [integration: repaints the chart when the slider is moved][int-timeline], [integration: shortens the chart by exactly the strip][int-timeline], [unit: takes each mark's value at the selected row rather than reducing][time-stops], [unit: unions every frame's stops, ascending and deduped][time-stops], [unit: resolves the timestamp against each frame's own row dimension][time-stops], [unit: is hidden on instant data][ed-timeline], [unit: stays visible on instant data once it is already on][ed-timeline], [unit: labels the main row Value under the time slider, on nodes and edges alike][tip-model] | [timeline.json][db-timeline] · [#1 live][live-timeline-1] vs [#5 live][live-timeline-5] (chord, the variant that morphs) · [#3 opts][live-opt-3]                                     |
| Nodes/Edges: stat fields reduced by the panel | "Calculation" — `reduceOptions.calcs`; `calcs[0]` colours a mark and thickens a sankey/chord ribbon, the rest are tooltip rows | Supported (different route) | [unit: normalizeRelationsCalcs / secondaryStatsOf][wide-conv]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | [all-options.json][db-opts] · [#2 opts][live-opt-2]                                                                                                                                  |
| —                                             | "Chart type" (Graph / Sankey / Chord) — panel `seriesType`                                                                     | ECharts-only                | [canvas: the same nodes and links as flow ribbons (four bars in columns, ribbons weighted by value)][canvas-sankey], [canvas: the same nodes and links as a ring of arcs (four arcs, chords weighted by value)][canvas-chord], [unit: buildOption per variant][rel-chart]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | [sankey.json][db-sankey] · [#1 live][live-sankey-1] vs [#5 live][live-sankey-5], [chord.json][db-chord] · [#1 live][live-chord-1] vs [#2 live][live-chord-2] · [#1 opts][live-opt-1] |
| —                                             | "Editor mode" (Default / Advanced) — tiers the options pane itself                                                             | ECharts-only                | [unit: the tier and its reset name the same options][rel-tier]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | —                                                                                                                                                                                    |
...
**`force.layoutAnimation` is the graph's only animation control, and that is deliberate.**
The shared root `animation.enabled` is _hidden_ on this variant because ECharts ignores it
there: `SankeyView` gates a clip-path reveal on `isAnimationEnabled()` and `ChordView`
...
arc in place, `SankeyView.render` writes every shape imperatively (`curve.setShape`) with no
diff at all, and a `graph` tweens only symbol size and position — neither of which is
...
| -------- | ----------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| Default  | Flow direction    | `series.sankey.orient`              | [canvas: vertical flow (bars in rows, each label below its bar)][canvas-sankey], [unit: getSankeyOrient][sankey-opts]           | [sankey.json][db-sankey] · [#2 live][live-sankey-2] · [#20 opts][live-opt-20] |
| Default  | Node alignment    | `series.sankey.nodeAlign`           | [unit: getSankeyNodeAlign][sankey-opts]                                                                                         | [sankey.json][db-sankey] · [#9 live][live-sankey-9] · [#19 opts][live-opt-19] |
| Advanced | Node width        | `series.sankey.nodeWidth`           | [canvas: node width 32 and gap 20 (wider bars, further apart)][canvas-sankey], [unit: getSankeySeries][sankey-opts]             | [sankey.json][db-sankey] · [#9 live][live-sankey-9] · [#21 opts][live-opt-21] |
| Advanced | Node gap          | `series.sankey.nodeGap`             | [canvas: node width 32 and gap 20 (wider bars, further apart)][canvas-sankey], [unit: getSankeySeries][sankey-opts]             | [sankey.json][db-sankey] · [#9 live][live-sankey-9] · [#22 opts][live-opt-22] |
| Advanced | Ribbon curveness  | `series.sankey.lineStyle.curveness` | [unit: getSankeyLinkStyle][sankey-opts]                                                                                         | [all-options.json][db-opts] · [#23 opts][live-opt-23]                         |
| Advanced | Ribbon opacity    | `series.sankey.lineStyle.opacity`   | [canvas: ribbon opacity 0.7 (ribbons nearly solid over the background)][canvas-sankey], [unit: getSankeyLinkStyle][sankey-opts] | [sankey.json][db-sankey] · [#9 live][live-sankey-9] · [#24 opts][live-opt-24] |
| Advanced | Layout iterations | `series.sankey.layoutIterations`    | [unit: getSankeySeries][sankey-opts]                                                                                            | [all-options.json][db-opts] · [#25 opts][live-opt-25]                         |
...
and [canvas: long names on a vertical flow (each truncated, none over the next bar)][canvas-sankey].
...
| Tier     | Option            | ECharts key                      | Regression test                                                                                                                 | Demo panel                                                                 |
| -------- | ----------------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| Advanced | Start angle       | `series.chord.startAngle`        | [canvas: start angle 0 and counter-clockwise (the ring rotated and reversed)][canvas-chord], [unit: getChordSeries][chord-opts] | [chord.json][db-chord] · [#7 live][live-chord-7] · [#26 opts][live-opt-26] |
| Advanced | Clockwise         | `series.chord.clockwise`         | [canvas: start angle 0 and counter-clockwise (the ring rotated and reversed)][canvas-chord], [unit: getChordSeries][chord-opts] | [chord.json][db-chord] · [#7 live][live-chord-7] · [#27 opts][live-opt-27] |
| Advanced | Arc gap           | `series.chord.padAngle`          | [canvas: pad angle 12 (wide gaps between arcs)][canvas-chord], [unit: getChordSeries][chord-opts]                               | [chord.json][db-chord] · [#7 live][live-chord-7] · [#28 opts][live-opt-28] |
| Advanced | Minimum arc angle | `series.chord.minAngle`          | [unit: getChordSeries][chord-opts]                                                                                              | [all-options.json][db-opts] · [#29 opts][live-opt-29]                      |
| Advanced | Ribbon opacity    | `series.chord.lineStyle.opacity` | [unit: getChordLinkStyle][chord-opts]                                                                                           | [chord.json][db-chord] · [#7 live][live-chord-7] · [#30 opts][live-opt-30] |
...
Keeps most of the standard field-config set (Color, Unit, Decimals, Min, Max, Field
min/max, Thresholds, Value mappings, Data links, Filterable, and Display name as an
override), customizing Color (PaletteClassic, byValue only — `bySeriesSupport: false`)
and un-hiding Filterable from the defaults section. Core's Node graph keeps the full set
too, but routes stat units through its own panel options rather than the standard Unit.

Three depart from "keep everything", each because the family's contract gives it nothing
to act on — see `RELATIONS_FIELD_OPTIONS` / `RELATIONS_DISABLED_FIELD_OPTIONS`:

- **Actions** and **No value** are unregistered outright (`disableStandardOptions`).
- **Display name** is `hideFromDefaults: true`, so it is an override-only property.

| Option         | Meaningful here?  | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Regression test                                                                                                                                                                                                                                | Demo panel                                                                                     |
| -------------- | ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| Color scheme   | **Yes**           | The load-bearing one, and now plain field behaviour: a mark's colour is `field.display(value).color`, which `applyFieldOverrides` already resolved, so all eight modes work and a `byName` override targets one node or one edge. A node derived from an edge's endpoints is declared as a field above the panel too ([derived nodes][derived]), so it behaves the same; only where that pre-pass cannot run does it fall back to the classic palette by position.                                                                                                                                                                                                                                                    | [canvas: a color field per node (blue, green, yellow and red symbols)][canvas-graph], [canvas: a byName color override (DB drawn red, the other three on the palette)][canvas-overrides], [unit: node colour / edge colour][wide-conv]         | [graph-wide.json][db-wide] · [#13 live][live-wide-13] · [#34 opts][live-opt-34]                |
| Unit           | **Yes**           | Per mark, not per frame: a hovered node or edge formats with its own field's unit, in the tooltip **and** in the "Show node values" label. Two nodes of one graph can differ, which the row form cannot express.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | [unit: per-mark formatting][tip-model], [e2e: emitted model][tip-emit]                                                                                                                                                                         | [per-mark-tooltip-links.json][db-marks], [graph-wide.json][db-wide] · [#14 live][live-wide-14] |
| Decimals       | **Yes**           | Same path as Unit.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | [unit: per-mark formatting][tip-model]                                                                                                                                                                                                         | [per-mark-tooltip-links.json][db-marks]                                                        |
| Value mappings | **Yes**           | Applied through the field's display processor, so a mapping targets **one mark** — a node can read `Healthy` while its neighbour reads a number — and reaches the "Show node values" label as well as the tooltip. A mapping carrying a `color` also recolours the mark, via the same `field.display` call. [value-mappings.json][db-value-mappings]                                                                                                                                                                                                                                                                                                                                                                  | [unit: value mappings][mappings]                                                                                                                                                                                                               | [value-mappings.json][db-value-mappings] · [#36 opts][live-opt-36]                             |
...
| Filterable     | **Yes**           | The **gate** on the pinned footer's ad-hoc filter buttons, matching core's own table cells (`DefaultCell`): a mark whose field does not carry it offers no filter at all, since a filter written under a key the datasource lacks either does nothing or empties the dashboard. Registered `hideFromDefaults: false` (`STANDARD_FILTERABLE_OPTION`) so "every mark in this panel" is one switch — core hides it, which on this family would mean one override per node **and** per edge. A mark with no field of its own — a derived node — takes the **edges'** answer, since a node's filters are written under the edges' endpoint keys; a node the [pre-pass][derived] declared answers with its own field first. | [e2e: offers no filter button at all when nothing is filterable][filters-test], [e2e: offers filters on a node the response only implied][filters-test], [unit: hides the label-derived filters when the field is not filterable][tip-overlay] | [devcortex-wide.json][db-devcortex]                                                            |
...
| Thresholds     | Marginal          | Reachable only as a by-value color scheme (`color.mode: thresholds`); there is no `markLine` equivalent because there are no axes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | —                                                                                                                                                                                                                                              | [colour-domain.json][db-colour-domain] · [#35 opts][live-opt-35]                               |
| Field min/max  | **Yes**           | Switches the range above from "across every field in the response" to "this field alone", which changes the question a percentage threshold answers — from how a node compares to its neighbours to how it compares to its own recent range. Visibly so: with it on, every mark in [colour-domain.json][db-colour-domain] sits at ~50% of its own band and lands on one step.                                                                                                                                                                                                                                                                                                                                         |
| Display name   | **Override only** | A node's name **is** its field's display name: the reader takes `config.displayName` (the row form's `title`) and falls back to the field name, so a `byName` override renames one node. Edges keep their field name — an edge's display name carries its labels, `e1 {source="a", target="b"}`. `hideFromDefaults` because a panel-wide value would rename every node to the same string; a `byName` override renaming one node is the useful form.                                                                                                                                                                                                                                                                  | [unit: one node per field, titled by displayName][wide-conv]                                                                                                                                                                                   | [graph-wide.json][db-wide] · [#8 live][live-wide-8]                                            |

Not registered, deliberately:

- **Actions** — `config.actions` is read **nowhere** in this plugin, so the property was
  offered and entirely inert. Unregistered rather than left in the Fields tab. If it is
  ever supported, its home is the pinned tooltip footer beside `config.links`
  (`tooltip/model.ts`), which is the only per-mark affordance surface the family has.
- **No value** — see the table above: reachable by no input this family renders.
- **"Color series by"** (`bySeriesSupport: false`) — writes `color.seriesBy`, read only by
  `getFieldSeriesColor`, which this family never reaches. A relations mark is one node or
  one edge coloured by `calcs[0]`, so the radio changed nothing. Implementing it would
  also duplicate "Calculation" and let a node be coloured by its max while labelled with
  its mean.
- **`tooltip.sort`** and **`tooltip.hideZeros`** — added unconditionally by
  `commonOptionsBuilder.addTooltipOptions` and gated by core on `tooltip.mode === 'multi'`,
  which `singleTooltipOnly` removes from the mode picker (a relations hover is one node or
  one link, so "All" has nothing to list). Both had no reachable state in which they could
  act, so the module strips them with `removeOption`. Pinned by `optionSurface.test.ts`.
- **`reduceOptions`** is registered (`addRelationsStatOptions`): `calcs[0]` is a
  ...
  Nothing is truncated: only `calcs[0]` is singular, because it is the number that colours a
  mark and sets sankey/chord ribbon thickness. It does **not** size a graph node or widen a
  graph edge — those are `custom.nodeRadius` and `custom.lineWidth`, per-mark field config.
  Sizing a node by its value is a gap: `todo/relations-node-size-by-value.md`. **Hidden while the "Time slider" switch is on**: at one selected
...
- **Gradient link colour needs the Fixed layout.** zrender resolves a non-global gradient
  against the shape's bounding box, so `x: 0 -> x2: 1` runs source-to-target only if the
  source happens to sit on the left. Under force or circular the positions do not exist
  until after ECharts has laid the graph out, so orienting the blend would be a coin flip
  and half the edges would report their direction backwards; the mode degrades to the
  source colour there instead. It is the _layout_, not the option, that decides — see
  `makeEdgeGradientResolver`, [canvas: gradient link color on a fixed layout (each line
  blends its source colour into its target)][canvas-graph] and [integration: a gradient
  link colour is emitted only where the layout knows the positions][int-layout].
...
[live-timeline-1]: http://localhost:3001/d/echarts-relations-timeline?viewPanel=1
[live-timeline-5]: http://localhost:3001/d/echarts-relations-timeline?viewPanel=5
