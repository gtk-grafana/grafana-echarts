...
//
// Adaptations from core: `useStyles2`/`InlineToast`/`Tooltip` are imported from
// the public `@grafana/ui` entry, and `clsx` is replaced by `@emotion/css`'s `cx`
// (clsx is not a plugin dependency).
...
export const VizTooltipRow = ({
  ...
  const fallbackCopyToClipboard = (text: string, type: LabelValueTypes) => {
    ...
    textarea.select();
    try {
      // eslint-disable-next-line @typescript-eslint/no-deprecated -- intentional clipboard fallback (copied from core)
            ...
            <>
              <Tooltip content={label} interactive={false} show={showLabelTooltip}>
                <>
                  ...
                  <div
                    className={cx(styles.label, isActive ? styles.activeSeries : '', CAN_COPY ? styles.copy : '')}
      ...
      <div className={styles.valueWrapper}>
        ...
        {!isPinned ? (
          ...
          <>
            ...
            <div
              className={cx(styles.value, CAN_COPY ? styles.copy : '')}
