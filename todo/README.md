# Open work

This directory contains current work and design references. Completed implementation records do not belong here because Git history keeps them.

## Release gates

- [Relations persistence end-to-end coverage](./relations-persistence-e2e.md)

## Post-release plugin work

- [Edge-label layout unit tests](./edge-label-layout-unit-tests.md)
- [Graph-wide design history](./graph-wide-history.md)
- [Hide in area gaps](./hide-from-area.md)
- [Hierarchy color schemes](./hierarchy-color-schemes.md)
- [ECharts lines series](./lines.md)
- [Multiple frames in categorical converters](./multiple-frames.md)
- [Relations label wrapping](./relations-label-wrap-broken.md)
- [Relations node size by value](./relations-node-size-by-value.md)
- [Relations null edge weight](./relations-null-edge-weight.md)

## Upstream work

- [Canvas snapshot path duplication](./canvas-snapshot-props-path.md)
- [Chord fixed edge colors](./relations-chord-edge-color.md)
- [Conditional relations field configuration](./relations-conditional-field-config.md)
- [Relations pre-pass registration test](./relations-prepass-registration-test.md)
- [Relations scrub animation](./relations-scrub-animation.md)
